...
TRIM(IR.firstname) AS firstname, TRIM(IR.lastname) AS lastname,
TRIM(EML.line1) AS emailaddress,
TRIM(NVL(PHN.phone,'')) AS cellphone
FROM
cc_current_students_vw DIR INNER JOIN id_rec IR ON DIR.id = IR.id
...
INNER JOIN acad_cal_rec CAL ON SSR.sess = CAL.sess
AND SSR.yr = CAL.yr
AND TODAY BETWEEN CAL.beg_date - 30 UNITS DAY AND CAL.end_date + 15 UNITS
DAY
AND CAL.subsess = ''
AND CAL.prog = 'UNDG'
LEFT JOIN aa_rec EML ON DIR.id = EML.id
AND EML.aa = 'EML1'
AND TODAY BETWEEN EML.beg_date AND NVL(EML.end_date, TODAY)
LEFT JOIN aa_rec PHN ON DIR.id = PHN.id
AND PHN.aa = 'CELL'
